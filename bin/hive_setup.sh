...
exit_f(){
    ...
    echo "############################### -- TDSS :: Hive Deployment -- $(tput setaf 1)Failed$(tput sgr0)"
    exit 1;
}
exit_s(){
	echo "############################### -- TDSS :: Hive Deployment -- $(tput setaf 2)Completed$(tput sgr0)"
}
...
install_hive(){
	...
	set -e
	if [[ $3 == 'NIL' ]]; then
		scp ../libs/apache-hive*.tar.gz "$2@$1:~/"
		#scp ../libs/db-derby*.tar.gz "$2@$1:~/"
		scp ../libs/mysql-connector-java.jar "$2@$1:~/"

		scp ../conf/hive/generated_hive_config/* "$2@$1:~/"
		
		ssh "$2@$1" 'bash -s' < ../sbin/install_hive.sh
	elif [[ $3 == 'KEY' ]]; then
		scp -i "$4" ../libs/apache-hive*.tar.gz "$2@$1:~/"
		#scp -i "$4" ../libs/db-derby*.tar.gz "$2@$1:~/"
		scp -i "$4" ../libs/mysql-connector-java.jar "$2@$1:~/"

		scp -i "$4" ../conf/hive/generated_hive_config/* "$2@$1:~/"
		
		ssh -i "$4" "$2@$1" 'bash -s' < ../sbin/install_hive.sh
	elif [[ $3 == 'PASS' ]]; then
		sshpass -p "$4" scp ../libs/apache-hive*.tar.gz "$2@$1:~/"
		#sshpass -p "$4" scp ../libs/db-derby*.tar.gz "$2@$1:~/"
		sshpass -p "$4" scp ../libs/mysql-connector-java.jar "$2@$1:~/"

		sshpass -p "$4" scp ../conf/hive/generated_hive_config/* "$2@$1:~/"

		sshpass -p "$4" ssh "$2@$1" 'bash -s' < ../sbin/install_hive.sh
	fi
	set +e
}
...
echo "-------------------------------"
echo "Generating Hive configuration"
if [[ ! -z "$(ls -A ../conf/hive/generated_hive_config/)" ]]; then
	echo "Clearing pre-generated Hive configuration"
	rm ../conf/hive/generated_hive_config/*
fi

cp ../conf/hive/config_template/hive-site.xml ../conf/hive/generated_hive_config/
sed -i "s@#MYSQL_SERVER#@$mysql_server_ip_final@" ../conf/hive/generated_hive_config/hive-site.xml
sed -i "s@#PORT#@$mysql_port_final@" ../conf/hive/generated_hive_config/hive-site.xml
sed -i "s@#MYSQL_USER#@$mysql_user_final@" ../conf/hive/generated_hive_config/hive-site.xml
sed -i "s@#MYSQL_PASSWORD#@$mysql_password_final@" ../conf/hive/generated_hive_config/hive-site.xml

sed -i "s@#HIVE_METASTORE#@$hive_metastore_db_name_final@" ../conf/hive/generated_hive_config/hive-site.xml

scp ../conf/hive/config_template/jpox.properties ../conf/hive/generated_hive_config/

# update sbin/install_hive.sh with install_dir
set -e
sed -i "s|INSTALL_DIRECTORY=.*|INSTALL_DIRECTORY=$install_dir_final|g" ../sbin/install_hive.sh
set +e

echo "Generating Hive configuration -$(tput setaf 2) Done $(tput sgr0)"

## ------------------------------------------------------ Check MySQL and Hadoop ------------------------------------------------------
...
#echo "-------------------------------"
#echo "Hive Installation"
#install_hive "$hive_server_ip_final" "$hive_server_username_final" "$hive_server_acc_typ_final" "$hive_server_acc_tkn_final"
#echo "Hive Installation -$(tput setaf 2) Done $(tput sgr0)"

echo "-------------------------------"
echo "Hive installation"
for server in ${hive_server_ip_final[@]}
do
        install_hive "$server" "$hive_server_username_final" "$hive_server_acc_typ_final" "$hive_server_acc_tkn_final"
done
echo "Hiveinstallation - $(tput setaf 2)Done$(tput sgr0)"


#Initialize Hive
run_cmd "${hive_server_ip_final[0]}" "$hive_server_username_final" "$hive_server_acc_typ_final" "$hive_server_acc_tkn_final" "schematool -dbType mysql -initSchema" "Initializing Hive Schema  in"
...
for server in ${hive_server_ip_final[@]}
do
        start_hive_metastore "$server" "$hive_server_username_final" "$hive_server_acc_typ_final" "$hive_server_acc_tkn_final"
done
echo "Hiveinstallation - $(tput setaf 2)Done$(tput sgr0)"
