...
exit_f(){
	...
	echo "############################### -- TDSS :: MySQL - Startup -- $(tput setaf 1)Failed$(tput sgr0)" 
    	exit;
}
...
# Start MySQL Service
start_mysql(){
	echo "$(tput setaf 3)Starting MySQL Service in '$1'$(tput sgr0)"
	set -e
	if [[ $3 == 'NIL' ]]; then
		ssh "$2@$1" 'bash -s' < ../sbin/start_mysqld.sh
	elif [[ $3 == 'KEY' ]]; then
		ssh -i "$4" "$2@$1" 'bash -s' < ../sbin/start_mysqld.sh
	elif [[ $3 == 'PASS' ]]; then
		sshpass -p "$4" ssh "$2@$1" 'bash -s' < ../sbin/start_mysqld.sh
	fi
	set +e
}
...
start_mysql "$mysql_server_ip_final" "$mysql_server_username_final" "$mysql_server_acc_typ_final" "$mysql_server_acc_tkn_final"
echo "Starting MySQL Service -$(tput setaf 2) Done $(tput sgr0)"

echo "############################### -- TDSS :: MySQL - Startup -- $(tput setaf 2)Done$(tput sgr0)"
