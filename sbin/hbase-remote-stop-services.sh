...
exit_f(){
    ...
    echo "############################### -- TDSS :: HBase Services - Stop -- $(tput setaf 1)Failed$(tput sgr0)"
    exit 1;
}
exit_s(){
	echo "############################### -- TDSS :: HBase Services - Stop -- $(tput setaf 2)Completed$(tput sgr0)"
}
...
# Stop HBase
stop_hbase(){
	echo "$(tput setaf 3)Stopping HBase in '$1'$(tput sgr0)"
	set -e
	if [[ $3 == 'NIL' ]]; then
		ssh "$2@$1" 'bash -s' < ../sbin/stop_hbase_services.sh
	elif [[ $3 == 'KEY' ]]; then
		ssh -i "$4" "$2@$1" 'bash -s' < ../sbin/stop_hbase_services.sh
	elif [[ $3 == 'PASS' ]]; then
		sshpass -p "$4" ssh "$2@$1" 'bash -s' < ../sbin/stop_hbase_services.sh
	fi
	set +e
}
...
if [[ $res != *"not running"* ]]; then 
	stop_hbase "$hbase_server_ip_final" "$hbase_server_username_final" "$hbase_server_auth_mech_final" "$hbase_server_auth_tkn_final"
else
	echo "$(tput setaf 3)Server '$hbase_server_ip_final'$(tput sgr0)"
	echo "$res"
fi
# echo "HBase Service -$(tput setaf 2) Stopped $(tput sgr0)"
