...
echo "############################### -- TDSS :: Zookeeper Service - Startup"
curr_dir=$(pwd)
...
exit_f(){
    ...
    echo "############################### -- TDSS :: Zookeeper Service - Startup -- $(tput setaf 1)Failed$(tput sgr0)"
    exit 1;
}
exit_s(){
	echo "############################### -- TDSS :: Zookeeper Service - Startup -- $(tput setaf 2)Completed$(tput sgr0)"
}
...
# Start Zookeeper
start_zookeeper(){
	echo "$(tput setaf 3)Starting Zookeeper in '$1'$(tput sgr0)"
	set -e
	if [[ $3 == 'NIL' ]]; then
		ssh "$2@$1" 'bash -s' < ../sbin/start_zookeeper_quorum.sh
	elif [[ $3 == 'KEY' ]]; then
		ssh -i "$4" "$2@$1" 'bash -s' < ../sbin/start_zookeeper_quorum.sh
	elif [[ $3 == 'PASS' ]]; then
		sshpass -p "$4" ssh "$2@$1" 'bash -s' < ../sbin/start_zookeeper_quorum.sh
	fi
	set +e
}
...
do
	...
	if [[ $res == *"not running"* ]]; then 
		start_zookeeper "$srvr" "$zookeeper_server_username_final" "$zookeeper_server_acc_typ_final" "$zookeeper_server_acc_tkn_final"
	else
		echo "$(tput setaf 3)Server '$srvr'$(tput sgr0)"
		echo "$res"
	fi
done
# echo "Zookeeper Service -$(tput setaf 2) Started $(tput sgr0)"
